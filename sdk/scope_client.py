"""
SCOPE — Public Python SDK
=========================
Thin client for the SCOPE (Spatial Change Observation & Predictive Engine) API.
The analytics engine itself is proprietary; this SDK gives you full programmatic
access to results.

    pip install requests

    from scope_client import Scope
    sw = Scope("https://api.scope.ai", token="...")
...
class Scope:
    ...
    def _post(self, path: str, payload: Dict) -> Dict:
        ...
        if r.status_code >= 400:
            raise ScopeError(f"{r.status_code}: {r.text[:300]}")
        return r.json()
    ...
    def analyze_ocean(self, source: str = "synthetic", **kwargs) -> Dict:
        """Sentinel-3 OLCI ocean-colour anomaly detection (blooms, plumes, CDOM).

        Returns named/classified events, chlorophyll, per-band attribution and
        figure URLs. See /api/ocean/analyze for parameters.
        """
        return self._post("/api/ocean/analyze", {"source": source, **kwargs})
